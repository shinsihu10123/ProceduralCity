...
const variants=[{id:'unit-basis-control',requiredLabor:false},{id:'unit-basis-production-required-labor',requiredLabor:true}];
...
function requiredWorkers(country,f){
  ...
  return {needed:Math.max(0,needed),perWorker,demandAnchor,expectedDemand,replenishment,unconstrainedPlan};
}
function installAblation(world){
  world.__rv07P12=new Map();
  const original=world.banking.originateCredit.bind(world.banking);
  world.banking.originateCredit=(country,month,signals)=>{
    const out=original(country,month,signals);const rows=[];
    for(const f of country.firms){if(f.active===false) continue;const canonicalDesired=finite(f.desiredWorkers);const d=requiredWorkers(country,f);f.desiredWorkers=d.needed;rows.push({firmId:f.id,industryId:f.industryId,canonicalDesiredWorkers:canonicalDesired,requiredDesiredWorkers:d.needed,perWorkerCapacity:d.perWorker,demandAnchor:d.demandAnchor,expectedDemand:d.expectedDemand,replenishment:d.replenishment,unconstrainedPlan:d.unconstrainedPlan});}
    world.__rv07P12.set(`${month}|${country.id}`,rows);return out;
  };
}
function runVariant(variant,scaleProfile,seed,horizon,collect=true){
  const world=createWorld(scaleProfile,seed);if(variant.requiredLabor)installAblation(world);else world.__rv07P12=new Map();const rows=[];
  for(let i=0;i<horizon;i++){world.stepMonth();if(collect)for(const c of world.countries){const goods=c.lastMarkets?.goods||{},ind=c.lastIndustry||{},mods=world.__rv07P12.get(`${world.month}|${c.id}`)||[];rows.push({variant:variant.id,scaleProfile,seed,month:world.month,countryId:c.id,economy:{unemployment:finite(c.macro?.unemployment),exits:finite(c.macro?.firmExits),wageArrears:finite(c.macro?.wageArrears),goodsFulfillment:ratio(finite(goods.nominalConsumption??c.macro?.consumption),finite(goods.desiredBudget)),inputShortage:finite(ind.inputShortageUnits),consumerOutput:finite(ind.sectorOutputs?.CONSUMER),gdp:finite(c.macro?.gdp),gdpResidual:gdpResidual(c.macro),activeFirms:finite(c.macro?.activeFirms)},labor:{totalWorkers:sum(c.firms.filter(f=>f.active!==false).map(f=>f.workers)),totalDesiredWorkers:sum(c.firms.filter(f=>f.active!==false).map(f=>f.desiredWorkers)),hires:finite(c.lastMarkets?.labor?.hires),layoffs:finite(c.lastMarkets?.labor?.layoffs),unfilled:finite(c.lastMarkets?.labor?.unfilled)},mods,ledgerOk:world.ledger.verifyCountry(c.id)?.ok===true});}}
...
const determinism=[];for(const v of variants)for(const s of scales){const seed=`ECON-RV07-P12-DET-${v.id}-${s}`,h=Math.min(3,months);const a=runVariant(v,s,seed,h,false).fingerprint,b=runVariant(v,s,seed,h,false).fingerprint,exact=JSON.stringify(a)===JSON.stringify(b);assert.ok(exact,`${v.id}/${s}: nondeterministic`);determinism.push({variant:v.id,scaleProfile:s,exact});}
...
const comparisons={};for(const s of scales){comparisons[s]={};for(const w of windows){const c=summary.find(x=>x.variant===variants[0].id&&x.scaleProfile===s&&x.window===w.id),a=summary.find(x=>x.variant===variants[1].id&&x.scaleProfile===s&&x.window===w.id);comparisons[s][w.id]={unemploymentDifference:a.meanUnemployment-c.meanUnemployment,exitDifference:a.totalExits-c.totalExits,wageArrearsDifference:a.meanWageArrears-c.meanWageArrears,goodsFulfillmentDifference:a.meanGoodsFulfillment-c.meanGoodsFulfillment,inputShortageDifference:a.meanInputShortage-c.meanInputShortage,consumerOutputRatio:ratio(a.meanConsumerOutput,c.meanConsumerOutput),gdpDifference:a.meanGdp-c.meanGdp,workerDifference:a.meanWorkers-c.meanWorkers,desiredWorkerDifference:a.meanDesiredWorkers-c.meanDesiredWorkers,unfilledDifference:a.meanUnfilled-c.meanUnfilled};}}
const candidateMods=rows.filter(r=>r.variant===variants[1].id).flatMap(r=>r.mods);const interventionEquationOk=candidateMods.every(m=>Number.isInteger(m.requiredDesiredWorkers)&&m.requiredDesiredWorkers>=0&&m.perWorkerCapacity>=0&&m.unconstrainedPlan>=0);const maxGdpResidual=Math.max(0,...rows.map(r=>Math.abs(r.economy.gdpResidual)));
const gates={deterministicReplayExact:determinism.every(x=>x.exact),allHealthy:runs.every(r=>r.health?.ok===true),completeCoverage:rows.length===variants.length*scales.length*seeds.length*months*4,interventionRowsPresent:candidateMods.length>0,interventionEquationValid:interventionEquationOk,ledgerCountriesOk:rows.every(r=>r.ledgerOk),gdpIdentityReconciled:maxGdpResidual<TOL,finiteRows:rows.every(r=>Number.isFinite(r.economy.unemployment)&&Number.isFinite(r.labor.totalWorkers))};gates.ok=Object.values(gates).every(Boolean);assert.ok(gates.ok,`WP-RV07-P12 gates failed: ${JSON.stringify(gates)}`);
console.table(summary.filter(x=>x.window==='FULL').map(x=>({variant:x.variant,scale:x.scaleProfile,u:+x.meanUnemployment.toFixed(4),exits:x.totalExits,arrears:+x.meanWageArrears.toFixed(1),fulfill:+x.meanGoodsFulfillment.toFixed(4),shortage:+x.meanInputShortage.toFixed(3),consumer:+x.meanConsumerOutput.toFixed(3),workers:+x.meanWorkers.toFixed(1),desired:+x.meanDesiredWorkers.toFixed(1),unfilled:+x.meanUnfilled.toFixed(2)})));
console.log('WP_RV07_P12_COMPARISON',JSON.stringify(comparisons));console.log('WP_RV07_P12_GATES',JSON.stringify(gates));
const payload={workPackage:'WP-RV07-P12',title:'Production-requirement labor-demand structural ablation',generatedAt:new Date().toISOString(),configuration:{scales,seeds,months},variants,determinism,gates,reconciliation:{maxGdpResidual},summary,comparisons,rows};if(outputJson){mkdirSync(dirname(outputJson),{recursive:true});writeFileSync(outputJson,JSON.stringify(payload,null,2));console.log('WP_RV07_P12_OUTPUT',outputJson);}
