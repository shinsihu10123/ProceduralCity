...
const variants=[{id:'unit-basis-control',guard:false},{id:'unit-basis-capacity-bound-layoff-guard',guard:true}];
...
function planningTerms(country,f){
  ...
  return {perWorker,demandAnchor,expectedDemand,replenishment,unconstrainedPlan,currentCapacityCap,capacityBoundAtCurrent:unconstrainedPlan>currentCapacityCap+TOL};
}
function installGuard(world){
  world.__rv07P14=new Map();
  ...
  world.banking.originateCredit=(country,month,signals)=>{
    ...
    for(const f of country.firms){
      ...
      const guardApplied=canonicalDesired+TOL<currentWorkers&&p.capacityBoundAtCurrent;
      const guardedDesired=guardApplied?currentWorkers:canonicalDesired;
      f.desiredWorkers=guardedDesired;
      mods.push({firmId:f.id,industryId:f.industryId,canonicalDesiredWorkers:canonicalDesired,currentWorkers,guardedDesiredWorkers:guardedDesired,guardApplied,preservedSlots:Math.max(0,guardedDesired-canonicalDesired),...p});
    }
    world.__rv07P14.set(`${month}|${country.id}`,mods);return result;
  };
}
function runVariant(variant,scaleProfile,seed,horizon,collect=true){
  const world=createWorld(scaleProfile,seed);if(variant.guard)installGuard(world);else world.__rv07P14=new Map();const rows=[];
  for(let i=0;i<horizon;i++){
    world.stepMonth();
    if(collect)for(const c of world.countries){const goods=c.lastMarkets?.goods||{},ind=c.lastIndustry||{},mods=world.__rv07P14.get(`${world.month}|${c.id}`)||[];rows.push({variant:variant.id,scaleProfile,seed,month:world.month,countryId:c.id,economy:{unemployment:finite(c.macro?.unemployment),exits:finite(c.macro?.firmExits),wageArrears:finite(c.macro?.wageArrears),goodsFulfillment:ratio(finite(goods.nominalConsumption??c.macro?.consumption),finite(goods.desiredBudget)),inputShortage:finite(ind.inputShortageUnits),resourceOutput:finite(ind.sectorOutputs?.RESOURCE),materialsOutput:finite(ind.sectorOutputs?.MATERIALS),consumerOutput:finite(ind.sectorOutputs?.CONSUMER),gdp:finite(c.macro?.gdp),gdpResidual:gdpResidual(c.macro)},labor:{workers:sum(c.firms.filter(f=>f.active!==false).map(f=>f.workers)),desiredWorkers:sum(c.firms.filter(f=>f.active!==false).map(f=>f.desiredWorkers)),hires:finite(c.lastMarkets?.labor?.hires),layoffs:finite(c.lastMarkets?.labor?.layoffs),unfilled:finite(c.lastMarkets?.labor?.unfilled)},mods,ledgerOk:world.ledger.verifyCountry(c.id)?.ok===true});}
  }
...
const determinism=[];for(const v of variants)for(const s of scales){const seed=`ECON-RV07-P14-DET-${v.id}-${s}`,h=Math.min(3,months);const a=runVariant(v,s,seed,h,false).fingerprint,b=runVariant(v,s,seed,h,false).fingerprint;const exact=JSON.stringify(a)===JSON.stringify(b);assert.ok(exact,`${v.id}/${s}: nondeterministic`);determinism.push({variant:v.id,scaleProfile:s,exact});}
...
const comparisons={};for(const s of scales){comparisons[s]={};for(const w of windows){const c=summary.find(x=>x.variant===variants[0].id&&x.scaleProfile===s&&x.window===w.id),a=summary.find(x=>x.variant===variants[1].id&&x.scaleProfile===s&&x.window===w.id);comparisons[s][w.id]={unemploymentDifference:a.meanUnemployment-c.meanUnemployment,exitDifference:a.totalExits-c.totalExits,wageArrearsDifference:a.meanWageArrears-c.meanWageArrears,goodsFulfillmentDifference:a.meanGoodsFulfillment-c.meanGoodsFulfillment,inputShortageDifference:a.meanInputShortage-c.meanInputShortage,resourceOutputRatio:ratio(a.meanResourceOutput,c.meanResourceOutput),materialsOutputRatio:ratio(a.meanMaterialsOutput,c.meanMaterialsOutput),consumerOutputRatio:ratio(a.meanConsumerOutput,c.meanConsumerOutput),gdpDifference:a.meanGdp-c.meanGdp,workerDifference:a.meanWorkers-c.meanWorkers,desiredWorkerDifference:a.meanDesiredWorkers-c.meanDesiredWorkers,layoffDifference:a.totalLayoffs-c.totalLayoffs,guardApplied:a.guardApplied,preservedSlots:a.preservedSlots};}}
const candidateMods=rows.filter(r=>r.variant===variants[1].id).flatMap(r=>r.mods);const ruleValid=candidateMods.every(m=>m.guardedDesiredWorkers+TOL>=m.canonicalDesiredWorkers&&m.guardedDesiredWorkers<=Math.max(m.canonicalDesiredWorkers,m.currentWorkers)+TOL&&(!m.guardApplied||Math.abs(m.guardedDesiredWorkers-m.currentWorkers)<TOL)&&(!m.guardApplied||m.capacityBoundAtCurrent));const maxGdpResidual=Math.max(0,...rows.map(r=>Math.abs(r.economy.gdpResidual)));
const gates={deterministicReplayExact:determinism.every(x=>x.exact),allHealthy:runs.every(x=>x.health?.ok===true),completeCoverage:rows.length===variants.length*scales.length*seeds.length*months*4,interventionRowsPresent:candidateMods.length>0,guardRuleValid:ruleValid,guardNeverCreatesExpansionAboveCurrentStaff:candidateMods.every(m=>m.guardedDesiredWorkers<=Math.max(m.canonicalDesiredWorkers,m.currentWorkers)+TOL),ledgerCountriesOk:rows.every(r=>r.ledgerOk),gdpIdentityReconciled:maxGdpResidual<TOL,finiteRows:rows.every(r=>Number.isFinite(r.economy.unemployment)&&Number.isFinite(r.labor.workers))};gates.ok=Object.values(gates).every(Boolean);assert.ok(gates.ok,`WP-RV07-P14 gates failed: ${JSON.stringify(gates)}`);
console.table(summary.filter(x=>x.window==='FULL').map(x=>({variant:x.variant,scale:x.scaleProfile,u:+x.meanUnemployment.toFixed(4),exits:x.totalExits,arrears:+x.meanWageArrears.toFixed(1),fulfill:+x.meanGoodsFulfillment.toFixed(4),shortage:+x.meanInputShortage.toFixed(3),consumer:+x.meanConsumerOutput.toFixed(3),workers:+x.meanWorkers.toFixed(1),layoffs:x.totalLayoffs,guard:x.guardApplied,preserved:+x.preservedSlots.toFixed(0)})));
console.log('WP_RV07_P14_COMPARISON',JSON.stringify(comparisons));console.log('WP_RV07_P14_GATES',JSON.stringify(gates));
const payload={workPackage:'WP-RV07-P14',title:'Capacity-bound layoff guard structural ablation',generatedAt:new Date().toISOString(),configuration:{scales,seeds,months},variants,determinism,gates,reconciliation:{maxGdpResidual},summary,comparisons,rows};if(outputJson){mkdirSync(dirname(outputJson),{recursive:true});writeFileSync(outputJson,JSON.stringify(payload,null,2));console.log('WP_RV07_P14_OUTPUT',outputJson);}
