...
const VARIANTS = Object.freeze([
  Object.freeze({
    id: 'frozen-control',
    kind: 'control',
    description: 'Unchanged frozen v0.10 economic semantics.'
  }),
  Object.freeze({
    id: 'price-wage-basis',
    kind: 'candidate',
  ...
  return COUNTRY_SEEDS.map(seed => {
    ...
    if (variantId === 'price-wage-basis') {
      ...
      return {
        ...
        __rv07P2: {
          originalInitialPrice: oldPrice,
          derivedPriceBasis: wageBasis,
          impliedMultiplier: wageBasis / oldPrice
        }
      };
    }
...
function createVariantWorld(variantId, scaleProfile, seedText) {
  ...
  const replacement = transformedSeeds(variantId);
...
function rowFor(world, variant, scaleProfile, seed, country) {
  ...
  return {
    ...
    payroll: {
      paid: finite(payroll.payroll),
      unpaid: finite(payroll.unpaid),
      payments: finite(payroll.payments)
    },
    credit: {
...
const report = {
  ...
  gates: {
    ...
    controlGdpIdentityReconciled: controlRuns.flatMap(run => run.rows).every(r => Math.abs(r.economy.gdpIdentityResidual) <= 1e-7),
    candidateGdpIdentityReconciled: candidateRuns.flatMap(run => run.rows).every(r => Math.abs(r.economy.gdpIdentityResidual) <= 1e-7)
  }
};
