...
function installTopologicalSupplyAblation(world) {
  world.__rv07P8Stage = new Map();

  function produceFirm(f, country, month, metrics) {
    ...
    if (f.inputProduct) {
      ...
      if (consumedUnits > EPS && available > EPS) {
        ...
        if (consumedValue > EPS) {
          world.accounting.recordInputConsumption({ firm: f, month, amount: consumedValue, product, units: consumedUnits });
        }
      }
    }
  ...
  function procureGroup(country, month, metrics, buyers) {
    ...
    for (const buyer of [...buyers].sort((a, b) => a.id.localeCompare(b.id))) {
      ...
      let budgetRemaining = world.ledger.balance(buyer.accountId) * 0.42;
  ...
  world.supply.procureInputs = (country, month) => {
    ...
    for (const f of resources) stage.resourceOutput += produceFirm(f, country, month, metrics);
    procureGroup(country, month, metrics, materials);
    for (const f of materials) stage.materialsOutput += produceFirm(f, country, month, metrics);
    procureGroup(country, month, metrics, downstream);
    for (const f of downstream) {
      const out = produceFirm(f, country, month, metrics);
      if (f.industryId === 'CAPITAL') stage.capitalOutput += out;
      else if (f.industryId === 'CONSUMER') stage.consumerOutput += out;
    }

    stage.inputShortageUnits = metrics.inputShortageUnits;
    stage.b2bSpend = metrics.b2bSpend;
    stage.b2bTransactions = metrics.b2bTransactions;
    world.__rv07P8Stage.set(`${month}|${country.id}`, stage);
    return metrics;
  };

  // Production already occurred in topological stages inside procureInputs.
  world.supply.produce = (country, month, metrics) => metrics;
}
...
function rowFor(world, variant, scaleProfile, seed, country) {
  ...
  const sectorOutputs = industry.sectorOutputs || {};
  const stage = world.__rv07P8Stage?.get(`${world.month}|${country.id}`) || null;
  const firmShortage = sum((country.firms || []).map(f => Math.max(0, finite(f.supplyShortage))));
  const firmOutput = sum((country.firms || []).filter(f => f.active !== false).map(f => Math.max(0, finite(f.output))));
  const sectorOutput = sum(Object.values(sectorOutputs));
  const productionBoundErrors = (country.firms || []).filter(f => f.active !== false).map(f => {
  ...
  return {
    ...
    economy: {
      ...
      resourceOutput: finite(sectorOutputs.RESOURCE),
      materialsOutput: finite(sectorOutputs.MATERIALS),
      capitalOutput: finite(sectorOutputs.CAPITAL),
      consumerOutput: finite(sectorOutputs.CONSUMER)
    },
    stage,
    reconciliation: {
      shortageError: firmShortage - finite(industry.inputShortageUnits),
      sectorOutputError: firmOutput - sectorOutput,
      maxProductionBoundError: Math.max(0, ...productionBoundErrors)
    },
...
for (const variant of VARIANTS) {
  for (const scaleProfile of scales) {
    const seed = `ECON-RV07-P8-DETERMINISM-${variant.id}-${scaleProfile}`;
...
function aggregate(rs) {
  return {
    ...
    meanResourceOutput: mean(rs.map(r => r.economy.resourceOutput)),
    meanMaterialsOutput: mean(rs.map(r => r.economy.materialsOutput)),
    meanCapitalOutput: mean(rs.map(r => r.economy.capitalOutput)),
...
for (const scaleProfile of scales) {
  ...
  for (const window of windows) {
    ...
    const candidate = summary.find(x => x.variant === 'unit-basis-topological-same-month-supply' && x.scaleProfile === scaleProfile && x.window === window.id);
...
const maxSectorOutputError = Math.max(0, ...rows.map(r => Math.abs(r.reconciliation.sectorOutputError)));
const maxProductionBoundError = Math.max(0, ...rows.map(r => Math.abs(r.reconciliation.maxProductionBoundError)));
...
const gates = {
  ...
  productionBoundsRespected: maxProductionBoundError < 1e-7,
  ledgerCountriesOk: allLedgerOk,
  gdpIdentityReconciled: maxGdpResidual < 1e-7,
  finiteRows: rows.every(r => Number.isFinite(r.economy.unemployment) && Number.isFinite(r.economy.inputShortageUnits) && Number.isFinite(r.economy.gdp))
};
gates.ok = Object.values(gates).every(Boolean);
assert.ok(gates.ok, `WP-RV07-P8 hard gates failed: ${JSON.stringify(gates)}`);
...
const compactTable = summary.filter(x => x.window === 'FULL').map(x => ({
  ...
  materialsOutput: Number(x.meanMaterialsOutput.toFixed(3)),
...
console.log('WP_RV07_P8_COMPARISON', JSON.stringify(comparisons));
console.log('WP_RV07_P8_GATES', JSON.stringify(gates));
console.log('WP_RV07_P8_RECONCILIATION', JSON.stringify({ maxShortageError, maxSectorOutputError, maxProductionBoundError, maxGdpResidual }));

const payload = {
  generatedAt: new Date().toISOString(),
  workPackage: 'WP-RV07-P8',
  description: 'Diagnostic-only topological same-month supply sequencing ablation on the unit-basis candidate.',
  ...
  determinism,
  gates,
  reconciliation: { maxShortageError, maxSectorOutputError, maxProductionBoundError, maxGdpResidual },
  summary,
  comparisons,
  rows
};
...
if (outputJson) {
  ...
  console.log('WP_RV07_P8_OUTPUT', outputJson);
}
