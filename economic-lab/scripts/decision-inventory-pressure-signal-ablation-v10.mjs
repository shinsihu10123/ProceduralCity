...
const variants=[{id:'unit-basis-control',neutral:false},{id:'unit-basis-firm-decision-inventory-neutral',neutral:true}];
...
function installNeutralInventorySignal(world){
  world.__rv07P28={saved:new Map(),decisionRows:[]};
  ...
  world.supply.beginMonth=country=>{
    ...
    for(const f of country.firms.filter(x=>x.active!==false)){
      const actualInventory=Math.max(0,finite(f.inventory)),targetInventory=Math.max(EPS,finite(f.targetInventory,1));
      saved.push({firmId:f.id,actualInventory,targetInventory,objectiveInventoryPressure:Math.max(-1,Math.min(2.5,(actualInventory-targetInventory)/Math.max(1,targetInventory)))});
      f.inventory=targetInventory;
    }
    world.__rv07P28.saved.set(`${world.month}|${country.id}`,saved);return out;
  };
  const credit=world.banking.originateCredit.bind(world.banking);
  world.banking.originateCredit=(country,month,signals)=>{
    const key=`${month}|${country.id}`,saved=world.__rv07P28.saved.get(key)||[],byId=new Map(saved.map(x=>[x.firmId,x]));
    for(const f of country.firms.filter(x=>x.active!==false)){
      const s=byId.get(f.id);assert.ok(s,`${key}/${f.id}: inventory snapshot missing`);
      world.__rv07P28.decisionRows.push({month,countryId:country.id,firmId:f.id,industryId:f.industryId,objectiveInventoryPressure:s.objectiveInventoryPressure,perceivedInventoryPressure:finite(f.lastTrace?.perception?.inventoryPressure),selected:f.currentPlan?.name||f.lastTrace?.selected||null,hiringChange:finite(f.currentPlan?.hiringChange),negativeHiring:finite(f.currentPlan?.hiringChange)<-TOL,cashStress:finite(f.lastTrace?.perception?.cashStress),actualInventory:s.actualInventory,targetInventory:s.targetInventory});
      f.inventory=s.actualInventory;
    }
    return credit(country,month,signals);
  };
}
function runVariant(variant,scaleProfile,seed,horizon,collect=true){
  const world=createWorld(scaleProfile,seed);if(variant.neutral)installNeutralInventorySignal(world);else world.__rv07P28={decisionRows:[]};const rows=[];
  for(let i=0;i<horizon;i++){
    world.stepMonth();
    if(collect)for(const c of world.countries){const m=c.macro||{},g=c.lastMarkets?.goods||{},ind=c.lastIndustry||{},decisions=variant.neutral?world.__rv07P28.decisionRows.filter(r=>r.month===world.month&&r.countryId===c.id):c.firms.filter(f=>f.active!==false).map(f=>({negativeHiring:finite(f.currentPlan?.hiringChange)<-TOL,perceivedInventoryPressure:finite(f.lastTrace?.perception?.inventoryPressure),objectiveInventoryPressure:(finite(f.inventory)-finite(f.targetInventory))/Math.max(1,finite(f.targetInventory,1))}));rows.push({variant:variant.id,scaleProfile,seed,month:world.month,countryId:c.id,unemployment:finite(m.unemployment),exits:finite(m.firmExits),activeFirms:finite(m.activeFirms),wageArrears:finite(m.wageArrears),goodsFulfillment:ratio(finite(g.nominalConsumption??m.consumption),finite(g.desiredBudget)),inputShortage:finite(ind.inputShortageUnits),resourceOutput:finite(ind.sectorOutputs?.RESOURCE),materialsOutput:finite(ind.sectorOutputs?.MATERIALS),consumerOutput:finite(ind.sectorOutputs?.CONSUMER),negativeHiringShare:ratio(decisions.filter(r=>r.negativeHiring).length,decisions.length),meanPerceivedInventoryPressure:mean(decisions.map(r=>r.perceivedInventoryPressure)),meanObjectiveInventoryPressure:mean(decisions.map(r=>r.objectiveInventoryPressure)),gdp:finite(m.gdp),gdpResidual:gdpResidual(m),ledgerOk:world.ledger.verifyCountry(c.id)?.ok===true});}
  }
  const health=world.forceHealthCheck();assert.ok(health.ok,`${variant.id}/${scaleProfile}/${seed}: health failed`);return {rows,decisionRows:world.__rv07P28.decisionRows,health,digest:digest(world)};
}
const determinism=[];for(const v of variants)for(const s of scales){const seed=`ECON-RV07-P28-DET-${v.neutral?'N':'C'}-${s}`,h=Math.min(3,months),a=runVariant(v,s,seed,h,false).digest,b=runVariant(v,s,seed,h,false).digest,exact=a===b;assert.ok(exact,`${v.id}/${s}: nondeterministic`);determinism.push({variant:v.id,scaleProfile:s,exact});}
...
const gates={deterministicReplayExact:determinism.every(x=>x.exact),allHealthy:runs.every(r=>r.health?.ok===true),completeCoverage:rows.length===variants.length*scales.length*seeds.length*months*4,candidateDecisionRowsPresent:candidateDecisions.length>0,candidatePerceivedInventoryPressureNeutral:candidateDecisions.every(r=>Math.abs(r.perceivedInventoryPressure)<TOL),objectiveInventoryPressureActuallyPresent:candidateDecisions.some(r=>Math.abs(r.objectiveInventoryPressure)>0.2),ledgerCountriesOk:rows.every(r=>r.ledgerOk),gdpIdentityReconciled:maxGdpResidual<TOL,finiteRows:rows.every(r=>Number.isFinite(r.unemployment)&&Number.isFinite(r.consumerOutput))};gates.ok=Object.values(gates).every(Boolean);assert.ok(gates.ok,`WP-RV07-P28 gates failed: ${JSON.stringify(gates)}`);
console.table(summary.filter(x=>x.window==='FULL').map(x=>({variant:x.variant,scale:x.scaleProfile,u:+x.meanUnemployment.toFixed(4),exits:x.totalExits,arrears:+x.meanWageArrears.toFixed(1),fulfill:+x.meanGoodsFulfillment.toFixed(4),shortage:+x.meanInputShortage.toFixed(2),consumer:+x.meanConsumerOutput.toFixed(2),negHire:+x.meanNegativeHiringShare.toFixed(3),perceivedInv:+x.meanPerceivedInventoryPressure.toFixed(3),objectiveInv:+x.meanObjectiveInventoryPressure.toFixed(3)})));
console.log('WP_RV07_P28_COMPARISON',JSON.stringify(comparisons));console.log('WP_RV07_P28_GATES',JSON.stringify(gates));
const payload={workPackage:'WP-RV07-P28',title:'Firm decision inventory-pressure signal causal ablation',generatedAt:new Date().toISOString(),configuration:{scales,seeds,months},variants,determinism,gates,reconciliation:{maxGdpResidual},summary,comparisons,rows,candidateDecisionRows:candidateDecisions};if(outputJson){mkdirSync(dirname(outputJson),{recursive:true});writeFileSync(outputJson,JSON.stringify(payload,null,2));console.log('WP_RV07_P28_OUTPUT',outputJson);}
