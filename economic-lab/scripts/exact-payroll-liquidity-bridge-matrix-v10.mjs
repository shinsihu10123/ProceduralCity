...
function runVariant(variant,scaleProfile,seed,horizon,collect=true){
  ...
  const health=world.forceHealthCheck();assert.ok(health.ok,`${variant.id}/${scaleProfile}/${seed}: health failed`);return {variant:variant.id,scaleProfile,seed,rows,health,fingerprint:fingerprint(world),events:world.__rv07P19};
}
const determinism=[];for(const v of variants)for(const s of scales){const seed=`ECON-RV07-P19-DET-${v.scope}-${s}`,h=Math.min(3,months);const a=runVariant(v,s,seed,h,false).fingerprint,b=runVariant(v,s,seed,h,false).fingerprint,exact=JSON.stringify(a)===JSON.stringify(b);assert.ok(exact,`${v.id}/${s}: nondeterministic`);determinism.push({variant:v.id,scaleProfile:s,exact});}
