...
const variantTargets={
  'unit-basis-control':[],
  'unit-basis-resource-floor':['RESOURCE'],
  'unit-basis-materials-floor':['MATERIALS'],
  'unit-basis-consumer-floor':['CONSUMER'],
  'unit-basis-capital-floor':['CAPITAL'],
  'unit-basis-upstream-floor':['RESOURCE','MATERIALS'],
  'unit-basis-noncapital-floor':['RESOURCE','MATERIALS','CONSUMER']
};
...
function createWorld(scaleProfile,seedText){
  ...
  try{return new EconomicWorld(seedText,{scaleProfile,healthCheckInterval:0});}
  finally{COUNTRY_SEEDS.splice(0,COUNTRY_SEEDS.length,...original);}
}
...
function installFloor(world,variant){
  const targets=new Set(variantTargets[variant]);
  world.__rv07P33={applications:0,checks:[]};
  ...
  world.supply.planProduction=country=>{
    ...
    for(const [industryId,inputProduct] of stages){
      ...
      for(const f of active.filter(x=>x.industryId===industryId)){
        const cap=finite(f.capacity);
        const laborPerUnit=cap>EPS?finite(f.wage)*finite(f.workers)/cap:0;
        const inputPerUnit=finite(f.inputPerOutput)*upstream;
        const floor=laborPerUnit+inputPerUnit;
        const before=finite(f.price);
        let applied=false;
        if(targets.has(industryId)&&cap>EPS&&floor>before+TOL){
          f.price=floor;
          world.__rv07P33.applications+=1;
          applied=true;
        }
        world.__rv07P33.checks.push({month:world.month,countryId:country.id,firmId:f.id,industryId,targeted:targets.has(industryId),capacity:cap,before,after:finite(f.price),floor,applied});
      }
    }
    return out;
  };
...
function runVariant(variant,scaleProfile,seed,horizon,captureFingerprint=false){
  ...
  installFloor(world,variant);
  const rows=[];
  for(let i=0;i<horizon;i++){
    world.stepMonth();
    for(const c of world.countries){
      rows.push({
        variant,scaleProfile,seed,month:world.month,countryId:c.id,
  ...
  return {variant,scaleProfile,seed,rows,checks:world.__rv07P33?.checks||[],applications:world.__rv07P33?.applications||0,health,fingerprint:captureFingerprint?fingerprint(world):null};
}

const determinism=[];
for(const variant of variants)for(const scaleProfile of scales){
  const seed=`ECON-RV07-P33-DET-${variant}-${scaleProfile}`;
...
const runs=[];
for(const variant of variants)for(const scaleProfile of scales)for(const seed of seeds)runs.push(runVariant(variant,scaleProfile,seed,months,false));
const rows=runs.flatMap(r=>r.rows);
const checks=runs.flatMap(r=>r.checks.map(x=>({...x,variant:r.variant,scaleProfile:r.scaleProfile,seed:r.seed})));
const windows=[
...
const summary=[];
for(const variant of variants)for(const scaleProfile of scales)for(const w of windows){
  summary.push({variant,scaleProfile,window:w.id,...aggregate(rows.filter(r=>r.variant===variant&&r.scaleProfile===scaleProfile&&r.month>=w.from&&r.month<=w.to))});
}
const comparison={};
...
const nonTargetMutation=checks.some(x=>!x.targeted&&Math.abs(x.after-x.before)>TOL);
const targetedViolation=Math.max(0,...checks.filter(x=>x.targeted&&x.capacity>EPS).map(x=>Math.max(0,x.floor-x.after)));
...
const gates={
  ...
  everyNonControlVariantActivated:variants.filter(v=>v!=='unit-basis-control').every(v=>nonControlRuns.filter(r=>r.variant===v).reduce((s,r)=>s+r.applications,0)>0),
  nonTargetSectorsNeverDirectlyFloored:!nonTargetMutation,
  targetedFloorsSatisfied:targetedViolation<TOL,
  ledgerCountriesOk:rows.every(r=>r.ledgerOk),
  gdpIdentityReconciled:maxGdpResidual<TOL,
  finiteRows:rows.every(r=>Number.isFinite(r.unemployment)&&Number.isFinite(r.priceIndex))
};
gates.ok=Object.values(gates).every(Boolean);
assert.ok(gates.ok,`WP-RV07-P33 gates failed: ${JSON.stringify(gates)}`);

console.table(summary.filter(x=>x.window==='FULL').map(x=>({variant:x.variant,scale:x.scaleProfile,u:+x.meanUnemployment.toFixed(4),exits:x.totalExits,arrears:+x.meanWageArrears.toFixed(1),fulfill:+x.meanGoodsFulfillment.toFixed(4),shortage:+x.meanInputShortage.toFixed(2),resource:+x.meanResourceOutput.toFixed(1),materials:+x.meanMaterialsOutput.toFixed(1),consumer:+x.meanConsumerOutput.toFixed(1),price:+x.meanPriceIndex.toFixed(1),cash:+x.meanFirmCash.toFixed(1)})));
console.log('WP_RV07_P33_COMPARISON',JSON.stringify(comparison));
console.log('WP_RV07_P33_GATES',JSON.stringify(gates));
const payload={workPackage:'WP-RV07-P33',title:'Sector-selective variable-cost floor causal matrix',generatedAt:new Date().toISOString(),configuration:{variants,variantTargets,scales,seeds,months},determinism,gates,reconciliation:{targetedViolation,maxGdpResidual},applications:runs.map(r=>({variant:r.variant,scaleProfile:r.scaleProfile,seed:r.seed,applications:r.applications})),summary,comparison,rows};
if(outputJson){mkdirSync(dirname(outputJson),{recursive:true});writeFileSync(outputJson,JSON.stringify(payload,null,2));console.log('WP_RV07_P33_OUTPUT',outputJson);}
