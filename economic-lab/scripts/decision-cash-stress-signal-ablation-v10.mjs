...
const variants=[{id:'unit-basis-control',neutral:false},{id:'unit-basis-firm-decision-cash-neutral',neutral:true}];
...
function installNeutralCashSignal(world){
  world.__rv07P25={saved:new Map(),decisionRows:[]};
  const begin=world.supply.beginMonth.bind(world.supply);
  world.supply.beginMonth=country=>{
    const out=begin(country),saved=[];
    for(const f of country.firms.filter(x=>x.active!==false)){
      const actualCash=finite(f.cash),safeCash=Math.max(EPS,finite(f.safeCash,1));
      saved.push({firmId:f.id,actualCash,safeCash,objectiveCashStress:Math.max(0,Math.min(1.3,1-actualCash/safeCash))});
      f.cash=safeCash;
    }
    world.__rv07P25.saved.set(`${world.month}|${country.id}`,saved);
    return out;
  };
  const credit=world.banking.originateCredit.bind(world.banking);
  world.banking.originateCredit=(country,month,signals)=>{
    const key=`${month}|${country.id}`,saved=world.__rv07P25.saved.get(key)||[],byId=new Map(saved.map(x=>[x.firmId,x]));
    for(const f of country.firms.filter(x=>x.active!==false)){
      const s=byId.get(f.id);assert.ok(s,`${key}/${f.id}: missing saved cash`);
      const perceived=finite(f.lastTrace?.perception?.cashStress);
      world.__rv07P25.decisionRows.push({month,countryId:country.id,firmId:f.id,industryId:f.industryId,objectiveCashStress:s.objectiveCashStress,perceivedCashStress:perceived,selected:f.currentPlan?.name||f.lastTrace?.selected||null,hiringChange:finite(f.currentPlan?.hiringChange),negativeHiring:finite(f.currentPlan?.hiringChange)<-TOL});
      f.cash=s.actualCash;
      assert.ok(Math.abs(f.cash-world.ledger.balance(f.accountId))<TOL,`${key}/${f.id}: cached cash not restored to ledger`);
    }
    return credit(country,month,signals);
  };
}
function runVariant(variant,scaleProfile,seed,horizon,collect=true){
  const world=createWorld(scaleProfile,seed);if(variant.neutral)installNeutralCashSignal(world);else world.__rv07P25={decisionRows:[]};const rows=[];
  for(let i=0;i<horizon;i++){
    world.stepMonth();
    if(collect)for(const c of world.countries){const m=c.macro||{},g=c.lastMarkets?.goods||{},ind=c.lastIndustry||{},decision=variant.neutral?world.__rv07P25.decisionRows.filter(r=>r.month===world.month&&r.countryId===c.id):c.firms.filter(f=>f.active!==false).map(f=>({negativeHiring:finite(f.currentPlan?.hiringChange)<-TOL,objectiveCashStress:Math.max(0,Math.min(1.3,1-finite(f.cash)/Math.max(EPS,finite(f.safeCash,1)))),perceivedCashStress:finite(f.lastTrace?.perception?.cashStress)}));rows.push({variant:variant.id,scaleProfile,seed,month:world.month,countryId:c.id,unemployment:finite(m.unemployment),exits:finite(m.firmExits),activeFirms:finite(m.activeFirms),wageArrears:finite(m.wageArrears),goodsFulfillment:ratio(finite(g.nominalConsumption??m.consumption),finite(g.desiredBudget)),inputShortage:finite(ind.inputShortageUnits),resourceOutput:finite(ind.sectorOutputs?.RESOURCE),materialsOutput:finite(ind.sectorOutputs?.MATERIALS),consumerOutput:finite(ind.sectorOutputs?.CONSUMER),negativeHiringShare:ratio(decision.filter(r=>r.negativeHiring).length,decision.length),meanPerceivedCashStress:mean(decision.map(r=>r.perceivedCashStress)),meanObjectiveCashStress:mean(decision.map(r=>r.objectiveCashStress)),gdp:finite(m.gdp),gdpResidual:gdpResidual(m),ledgerOk:world.ledger.verifyCountry(c.id)?.ok===true});}
  }
  const health=world.forceHealthCheck();assert.ok(health.ok,`${variant.id}/${scaleProfile}/${seed}: health failed`);return {rows,decisionRows:world.__rv07P25.decisionRows,health,digest:digest(world)};
}
const determinism=[];for(const v of variants)for(const s of scales){const seed=`ECON-RV07-P25-DET-${v.neutral?'N':'C'}-${s}`,h=Math.min(3,months),a=runVariant(v,s,seed,h,false).digest,b=runVariant(v,s,seed,h,false).digest,exact=a===b;assert.ok(exact,`${v.id}/${s}: nondeterministic`);determinism.push({variant:v.id,scaleProfile:s,exact});}
const runs=[];for(const v of variants)for(const s of scales)for(const seed of seeds)runs.push({variant:v.id,scaleProfile:s,seed,...runVariant(v,s,seed,months,true)});const rows=[];for(const r of runs)for(const x of r.rows)rows.push(x);const candidateDecisions=runs.filter(r=>r.variant===variants[1].id).flatMap(r=>r.decisionRows);
...
const gates={deterministicReplayExact:determinism.every(x=>x.exact),allHealthy:runs.every(r=>r.health?.ok===true),completeCoverage:rows.length===variants.length*scales.length*seeds.length*months*4,candidateDecisionRowsPresent:candidateDecisions.length>0,candidatePerceivedCashStressNeutral:candidateDecisions.every(r=>Math.abs(r.perceivedCashStress)<TOL),objectiveStressActuallyPresent:candidateDecisions.some(r=>r.objectiveCashStress>0.2),ledgerCountriesOk:rows.every(r=>r.ledgerOk),gdpIdentityReconciled:maxGdpResidual<TOL,finiteRows:rows.every(r=>Number.isFinite(r.unemployment)&&Number.isFinite(r.consumerOutput))};gates.ok=Object.values(gates).every(Boolean);assert.ok(gates.ok,`WP-RV07-P25 gates failed: ${JSON.stringify(gates)}`);
console.table(summary.filter(x=>x.window==='FULL').map(x=>({variant:x.variant,scale:x.scaleProfile,u:+x.meanUnemployment.toFixed(4),exits:x.totalExits,arrears:+x.meanWageArrears.toFixed(1),fulfill:+x.meanGoodsFulfillment.toFixed(4),shortage:+x.meanInputShortage.toFixed(2),consumer:+x.meanConsumerOutput.toFixed(2),negHire:+x.meanNegativeHiringShare.toFixed(3),perceivedCash:+x.meanPerceivedCashStress.toFixed(3),objectiveCash:+x.meanObjectiveCashStress.toFixed(3)})));
console.log('WP_RV07_P25_COMPARISON',JSON.stringify(comparisons));console.log('WP_RV07_P25_GATES',JSON.stringify(gates));
const payload={workPackage:'WP-RV07-P25',title:'Firm decision cash-stress signal causal ablation',generatedAt:new Date().toISOString(),configuration:{scales,seeds,months},variants,determinism,gates,reconciliation:{maxGdpResidual},summary,comparisons,rows,candidateDecisionRows:candidateDecisions};if(outputJson){mkdirSync(dirname(outputJson),{recursive:true});writeFileSync(outputJson,JSON.stringify(payload,null,2));console.log('WP_RV07_P25_OUTPUT',outputJson);}
