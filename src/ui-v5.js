...
strip.innerHTML=`<button type="button" data-view="overview" class="active">조감</button><button type="button" data-view="street">가로</button><button type="button" data-view="center">도심</button><button type="button" data-view="reset">초기화</button>`
viewport.appendChild(strip)
const states={panel:'v5-panel-open',kpi:'v5-kpi-open',environment:'v5-environment-open',analysis:'v5-analysis-open',layer:'v5-layer-open',map:'v5-map-open'}
function closeTransient(except){for(const [key,className] of Object.entries(states)){if(key!==except&&key!=='kpi'&&key!=='environment')body.classList.remove(className)}toolbar.querySelectorAll('[data-v5-toggle]').forEach(button=>{const key=button.dataset.v5Toggle;button.setAttribute('aria-pressed',String(body.classList.contains(states[key])))})}
...
strip.addEventListener('click',event=>{const button=event.target.closest('[data-view]');if(!button)return;document.querySelector(`[data-camera="${button.dataset.view}"]`)?.click();strip.querySelectorAll('button').forEach(item=>item.classList.toggle('active',item===button))})
...
body.classList.add('v5-environment-open')
