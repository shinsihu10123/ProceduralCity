...
fn rec() -> SpatialRecord {
    SpatialRecord {
        stable_id: "place-1".into(),
        namespace: "space".into(),
        version: 1,
        owner: OWNER.into(),
...
fn admission_and_semantic_contract() {
    ...
    assert_eq!(
        validate_authority(OWNER, Origin::Observer),
        Err(SpaceError::UnauthorizedWrite)
    );
}
#[test]
fn hierarchy_and_address_boundaries() {
    assert_eq!(
        validate_cell(CellAddress {
            face: Face::PosX,
            level: 2,
            x: 3,
            y: 3
...
fn cross_face_is_deterministic() {
    ...
    assert_eq!(
        cross_face_neighbor(c, 1, 0).unwrap(),
        CellAddress {
            face: Face::NegZ,
            level: 2,
            x: 0,
...
fn geodesic_query_is_read_only_and_repeatable() {
    ...
    let x = geodesic(a, b, 10.0).unwrap();
    let y = geodesic(a, b, 10.0).unwrap();
    assert_eq!(x, y);
    assert!((x.0 - std::f64::consts::FRAC_PI_2 * 10.0).abs() < 1e-10);
}
#[test]
fn tangent_frame_is_orthogonal() {
    let (e, n, u) = tangent_frame(
        Face::PosZ,
        Position {
            u: 0.5,
            v: 0.5,
...
fn serialization_round_trip_preserves_state() {
    let r = rec();
    let s = serialize(&r);
    let rr = deserialize(&s, &r.lineage).unwrap();
    assert_eq!(r, rr);
    assert_eq!(serialize(&rr), s);
}
#[test]
fn acceptance_requires_every_member_and_evidence() {
    let passes = [true; 11];
    let evidence = [1u64; 11];
    let a = accept(&root(), &passes, &evidence).unwrap();
    assert!(a.closed);
    assert_eq!(a.member_ids, MEMBER_IDS);
    let mut missing = passes;
    missing[7] = false;
    assert_eq!(
        accept(&root(), &missing, &evidence),
        Err(SpaceError::MissingEvidence("S1.04.08"))
    );
    let mut noev = evidence;
    noev[10] = 0;
    assert_eq!(
        accept(&root(), &passes, &noev),
        Err(SpaceError::MissingEvidence("S1.04.11"))
    );
}
