...
impl SimulationCheckpoint {
    ...
    pub const fn new(tick: u64, world_seed: u64, running: bool) -> Self {
        Self {
            time: TimeSaveStateV1 {
                current_tick: tick,
                tick_duration_nanos: TickDuration::BASELINE.as_nanos(),
            },
            world_seed,
            running,
        }
    ...
    pub const fn with_time(time: TimeSaveStateV1, world_seed: u64, running: bool) -> Self {
        Self { time, world_seed, running }
    }

    #[must_use] pub const fn tick(self) -> Tick { Tick::new(self.time.current_tick) }
    #[must_use] pub const fn time(self) -> TimeSaveStateV1 { self.time }
    #[must_use] pub const fn world_seed(self) -> u64 { self.world_seed }
    #[must_use] pub const fn is_running(self) -> bool { self.running }
}
...
impl SimulationHost {
    ...
    pub const fn new(world_seed: u64) -> Self {
        Self { time: TimeService::new(TickDuration::BASELINE), world_seed }
    }
    ...
    pub fn restore(checkpoint: SimulationCheckpoint) -> Self {
        ...
        if checkpoint.running { time.start(); }
        Self { time, world_seed: checkpoint.world_seed }
    }

    pub const fn start(&mut self) { self.time.start(); }
    pub const fn pause(&mut self) { self.time.pause(); }
    #[must_use] pub const fn tick(&self) -> Tick { self.time.current_tick() }
    #[must_use] pub const fn world_time(&self) -> WorldTime { self.time.world_time() }
    #[must_use] pub const fn time_service(&self) -> &TimeService { &self.time }
    #[must_use] pub const fn world_seed(&self) -> u64 { self.world_seed }
    #[must_use] pub const fn is_running(&self) -> bool { self.time.is_running() }

    #[must_use]
    pub fn checkpoint(&self) -> SimulationCheckpoint {
        let time = self.time.save_state().expect("checkpoint requested at a tick boundary");
    ...
    pub fn deterministic_digest(&self) -> u64 {
        ...
        for byte in self.world_seed.to_le_bytes().into_iter()
...
mod tests {
    ...
    fn fixed_tick_progresses_one_step_at_a_time() {
        let mut host = SimulationHost::new(42); host.start();
    ...
    fn identical_seed_and_steps_produce_identical_digest() {
        let mut left = SimulationHost::new(7); let mut right = SimulationHost::new(7);
        left.start(); right.start();
        for _ in 0..10_000 { left.step().unwrap(); right.step().unwrap(); }
        assert_eq!(left.deterministic_digest(), right.deterministic_digest());
    }

    #[test]
    fn checkpoint_restore_preserves_digest_and_progression() {
        let mut uninterrupted = SimulationHost::new(42); uninterrupted.start();
        for _ in 0..20_000 { uninterrupted.step().unwrap(); }
        let mut first_half = SimulationHost::new(42); first_half.start();
        for _ in 0..10_000 { first_half.step().unwrap(); }
        let mut restored = SimulationHost::restore(first_half.checkpoint());
        for _ in 0..10_000 { restored.step().unwrap(); }
